...
namespace Direct3D12 {
            ...
            internal enum class PixEventType : int32_t
        {
            ...
            PIXEvent_SetMarker_NoArgs = 0x008
                                ...
                                internalstatic int32_t CalculateNoArgsEventSize(const std::string& message)
                (3 /* start marker, color, copy chunk size */ +
                message.Length / 4 /* 8 bytes / 2 bytes per character */ +
                1 /* nullptr terminator */ +
                1 /* end marker */) * 8;
                                        internalstatic void FormatNoArgsEventToBuffer(void* outputBuffer, PixEventType eventType, uint64_t color, const std::string& message)
                {
