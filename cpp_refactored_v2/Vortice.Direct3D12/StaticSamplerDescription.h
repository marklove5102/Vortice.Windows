...
namespace Direct3D12 {
        ...
        publicstruct StaticSamplerDescription
        {
                ...
                ShaderVisibility = shaderVisibility;
                        ...
                        public StaticSamplerDescription(uint32_t shaderRegister, Filter filter, TextureAddressMode addressU, TextureAddressMode addressV, TextureAddressMode addressW, float mipLODBias, uint32_t maxAnisotropy, ComparisonFunction comparisonFunction, StaticBorderColor borderColor, float minLOD, float maxLOD, ShaderVisibility shaderVisibility, uint32_t registerSpace)
                {
                Filter = filter;
                AddressU = addressU;
                AddressV = addressV;
                                        ...
                                        public StaticSamplerDescription(SamplerDescription samplerDescription, ShaderVisibility shaderVisibility, uint shaderRegister, uint registerSpace) : this()
            {
                ...
                // So if the sampler has a border color that matches one of those exactly, we can convert it{}; otherwise it's left as the {}.
