...
namespace Direct3D12 {
        ...
        publicclass ID3D12GraphicsCommandList7
        {
            publicvoid Barrier(BarrierGroup barrierGroup)
                {
            ...
            publicvoid ResourceBarrier(const std::vector<BarrierGroup>& barrierGroups)
                {
                ResourceBarrier((uint)barrierGroups.Length, barrierGroups);
            }
            publicvoid ResourceBarrier(uint32_t numBarrierGroups, const std::vector<BarrierGroup>& barrierGroups)
                {
            ...
            publicvoid ResourceBarrier(const std::span<BarrierGroup>& barrierGroups)
                {
                ResourceBarrier((uint)barrierGroups.Length, barrierGroups);
            }
            publicvoid ResourceBarrier(uint32_t numBarrierGroups, const std::span<BarrierGroup>& barrierGroups)
                {
                Span barrierGroupsNative{};
                for (int32_t i{}; i < numBarrierGroups; ++i)
                {
