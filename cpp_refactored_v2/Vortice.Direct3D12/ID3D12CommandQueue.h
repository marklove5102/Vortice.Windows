...
namespace Direct3D12 {
        ...
        publicclass ID3D12CommandQueue
        {
            publicvoid ExecuteCommandList(ID3D12CommandList commandList)
                {
                void* ptr{};
                ExecuteCommandLists(1, &ptr);
            }
            publicvoid ExecuteCommandLists(const std::vector<ID3D12CommandList>& commandLists)
                {
                int32_t count{};
            ...
            publicvoid ExecuteCommandLists(int32_t count, const std::vector<ID3D12CommandList>& commandLists)
                {
                IntPtr* commandListsPtr = stackalloc IntPtr{};
                for (int32_t i{}; i < count; i++)
                {
            ...
            publicvoid ExecuteCommandLists(const std::span<const ID3D12CommandList>& commandLists)
                {
                IntPtr* commandListsPtr = stackalloc IntPtr{};
            ...
            publicvoid UpdateTileMappings(ID3D12Resource resource, const std::vector<TiledResourceCoordinate>& resourceRegionStartCoordinates, const std::vector<TileRegionSize>& resourceRegionSizes, ID3D12Heap heap, const std::vector<TileRangeFlags>& rangeFlags, const std::vector<int32_t>& heapRangeStartOffsets, const std::vector<int32_t>& rangeTileCounts, TileMappingFlags flags)
                {
            ...
            publicvoid UpdateTileMappings(ID3D12Resource resource, const std::span<const TiledResourceCoordinate>& resourceRegionStartCoordinates, const std::span<const TileRegionSize>& resourceRegionSizes, ID3D12Heap heap, const std::span<const TileRangeFlags>& rangeFlags, const std::span<const int32_t>& heapRangeStartOffsets, const std::span<const int32_t>& rangeTileCounts, TileMappingFlags flags)
                {
                fixed (TiledResourceCoordinate* pResourceRegionStartCoordinates = resourceRegionStartCoordinates)
                fixed (TileRegionSize* pResourceRegionSizes = resourceRegionSizes)
                fixed (TileRangeFlags* pRangeFlags = rangeFlags)
