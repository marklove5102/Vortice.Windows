...
namespace Direct3D12 {
        ...
        publicstruct CpuDescriptorHandle : : IEquatable<CpuDescriptorHandle>
        {
            public static CpuDescriptorHandle get_Default() const { return {}; }
            ...
            publicvoid InitOffsetted(const CpuDescriptorHandle& @base, int32_t offsetScaledByIncrementSize)
                {
                InitOffsetted(ref this, @base, offsetScaledByIncrementSize);
            }
            publicvoid InitOffsetted(const CpuDescriptorHandle& @base, int32_t offsetInDescriptors, uint32_t descriptorIncrementSize)
                {
                InitOffsetted(ref this, @base, offsetInDescriptors, descriptorIncrementSize);
            }
            publicstatic void InitOffsetted(CpuDescriptorHandle& handle, const CpuDescriptorHandle& @base, int32_t offsetScaledByIncrementSize)
                {
                handle.Ptr = unchecked((nuint)((long)(@base.Ptr) + (long)(offsetScaledByIncrementSize)));
            }
            publicstatic void InitOffsetted(CpuDescriptorHandle& handle, const CpuDescriptorHandle& @base, int32_t offsetInDescriptors, uint32_t descriptorIncrementSize)
                {
