...
namespace Direct3D12 {
        ...
        publicclass ID3D12Resource
        {
            public: HeapProperties m_heapProperties{};
                public: HeapProperties get_HeapProperties() const { return GetHeapProperties(properties, out _);
                    return properties{};; }
            public: HeapFlags m_heapFlags{};
                public: HeapFlags get_HeapFlags() const { return GetHeapProperties(out _, heapFlags);
                    return heapFlags{};; }
            publicResult Map(uint32_t subresource, void* data) Map(subresource, default, data);
            ...
            publicuint64_t GetRequiredIntermediateSize(uint32_t firstSubresource, uint32_t numSubresources)
                {
                ResourceDescription desc{};
                    return requiredSize{};
                }
            }
                                                        publicstatic uint32_t CalculateSubResourceIndex(uint32_t mipSlice, uint32_t arraySlice, uint32_t planeSlice, uint32_t mipLevels, uint32_t arraySize)
                {
                return mipSlice + arraySlice * mipLevels + planeSlice * mipLevels * arraySize;
            }
            publicstatic void DecomposeSubresource(uint32_t subresource, uint32_t mipLevels, uint32_t arraySize, uint32_t& mipSlice, uint32_t& arraySlice, uint32_t& planeSlice)
                {
