...
namespace Direct3D12 {
        ...
        publicstruct ResourceBarrier
        {
                        public: const ResourceBarrierType Type{};
                        public: const ResourceBarrierFlags Flags{};
            private: const Union _union{};
            public: ResourceTransitionBarrier get_Transition() const { return _union.Transition; }
            public: ResourceAliasingBarrier get_Aliasing() const { return _union.Aliasing; }
            public: ResourceUnorderedAccessViewBarrier get_UnorderedAccessView() const { return _union.UnorderedAccessView; }
                ...
                _union = new Union { UnorderedAccessView = unorderedAccessView };
                                                ...
                                                publicstatic ResourceBarrier BarrierTransition(ID3D12Resource resource, ResourceStates stateBefore, ResourceStates stateAfter, uint32_t subresource, ResourceBarrierFlags flags)
                {
                return new ResourceBarrier(new ResourceTransitionBarrier(resource, stateBefore, stateAfter, subresource), flags);
            }
                                    publicstatic ResourceBarrier BarrierAliasing(const std::optional<ID3D12Resource>& resourceBefore, const std::optional<ID3D12Resource>& resourceAfter)
                {
                return new ResourceBarrier(new ResourceAliasingBarrier(resourceBefore, resourceAfter));
            }
                                publicstatic ResourceBarrier BarrierUnorderedAccessView(const std::optional<ID3D12Resource>& resource)
                {
