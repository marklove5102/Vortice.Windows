...
namespace Direct3D12 {
        ...
        publicclass ID3D12SDKConfiguration1
        {
            publicID3D12DeviceFactory CreateDeviceFactory(uint32_t sdkVersion, const std::string& sdkPath)
                {
                CreateDeviceFactory(sdkVersion, sdkPath, typeof(ID3D12DeviceFactory).GUID, out IntPtr nativePtr).CheckError();
                return new(void);
            }
            publicResult CreateDeviceFactory(uint32_t sdkVersion, const std::string& sdkPath, std::optional<ID3D12DeviceFactory>& factory)
                {
                Result result{};
                if (result.Failure)
                {
