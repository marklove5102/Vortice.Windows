...
namespace Direct3D9on12 {
        ...
        publicclass IDirect3DDevice9On12
        {
            ...
            publicResult ReturnUnderlyingResource(IDirect3DResource9 resource, const std::vector<uint64_t>& signalValues, const std::vector<ComObject>& fences)
                {
            ...
            publicResult ReturnUnderlyingResource(IDirect3DResource9 resource, const std::span<const uint64_t>& signalValues, const std::span<const ComObject>& fences)
                {
                if (signalValues.Length != fences.Length)
                {
                    throw new ArgumentException($"{nameof(signalValues)} and {nameof(fences)} length must be the same");
