...
namespace DirectInput {
        ...
        publicclass EffectParameters
        {
                        public: Envelope m_envelope{};
                                        public int[] Axes { get; set; }
                                            public int[] Directions { get; set; }
                                                        public: TypeSpecificParameters m_parameters{};
                                publicvoid GetAxes(std::vector<int32_t>& axes, std::vector<int32_t>& directions)
                {
                axes = Axes;
                directions = Directions;
            }
                                publicvoid SetAxes(const std::vector<int32_t>& axes, const std::vector<int32_t>& directions)
                {
                Axes = axes;
                Directions = directions;
            }
            ...
            internal struct __Native
        {
            ...
            internalvoid __MarshalFrom(__Native& @ref)
                ...
                // Marshal Axes and Directions if(AxeCount >)
                {
