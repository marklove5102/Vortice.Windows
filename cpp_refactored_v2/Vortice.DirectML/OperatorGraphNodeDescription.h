...
namespace DirectML {
        ...
        publicstruct OperatorGraphNodeDescription : : IGraphNodeDescription, IGraphNodeDescriptionMarshal
        {
                        public: GraphNodeType get_GraphNodeType() const { return GraphNodeType.Operator; }
                public: IDMLOperator m_operator{};
                public: std::optional<std::string> m_name{};
                publicoverride std::string ToString(void) $"Operator{(string.IsNullOrEmpty(Name) ? "" : $": Name={Name}")}";
