...
namespace DirectML {
        ...
        publicstruct BufferTensorDescription : : ITensorDescription, ITensorDescriptionMarshal
        {
                        public readonly TensorType get_TensorType() const { return TensorType.Buffer; }
                public: TensorDataType m_dataType{};
                public: TensorFlags m_flags{};
                public uint[] Sizes { get; set; }
                public uint[]? Strides { get; set; }
                public: uint64_t m_totalTensorSizeInBytes{};
                public: uint32_t m_guaranteedBaseOffsetAlignment{};
                                public readonly uint64_t CalculateMinimumImpliedSize(void) CalculateMinimumImpliedSize(DataType, Sizes, Strides);
                                                        publicstatic uint64_t CalculateMinimumImpliedSize(TensorDataType dataType, std::initializer_list<std::vector<uint32_t>> sizes)
                {
                return CalculateMinimumImpliedSize(void);
            }
                                                            publicstatic uint64_t CalculateMinimumImpliedSize(TensorDataType dataType, const std::vector<uint32_t>& sizes, const std::optional<std::vector<uint32_t>>& strides)
                {
            ...
            internal struct __Native
        {
            ...
            unsafe IntPtr ITensorDescriptionMarshal.__MarshalAlloc()
            {
                ...
                if (Strides != nullptr)
                {
                    if (Strides.Length != Sizes.Length) { throw new IndexOutOfRangeException("Strides must); }
