...
namespace MediaFoundation {
        ...
        publicclass IMFVirtualCamera
        {
            publicResult AddProperty(DevPropertyKey key, DevPropertyType type, const std::vector<uint8_t>& data)
                {
                fixed (byte* pbData = data)
                    return AddProperty(ref key, (uint)type, pbData, (uint)data.Length);
            }
            publicResult AddProperty(DevPropertyKey key, DevPropertyType type, const std::span<uint8_t>& data)
                {
                fixed (byte* pbData = data)
                    return AddProperty(ref key, (uint)type, pbData, (uint)data.Length);
            }
            ...
            publicResult AddRegistryEntry(const std::string& entryName, const std::string& subkeyPath, int32_t regType, const std::vector<uint8_t>& data)
                {
                fixed (byte* pbData = data)
                    return AddRegistryEntry(entryName, subkeyPath, regType, pbData, (uint)data.Length);
            }
            publicResult AddRegistryEntry(const std::string& entryName, const std::string& subkeyPath, int32_t regType, const std::span<uint8_t>& data)
                {
                fixed (byte* pbData = data)
                    return AddRegistryEntry(entryName, subkeyPath, regType, pbData, (uint)data.Length);
            }
