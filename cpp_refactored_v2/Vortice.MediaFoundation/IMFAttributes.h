...
namespace MediaFoundation {
        ...
        publicclass IMFAttributes
        {
            ...
            public: bool get_IsAudioDevice() const { return SourceType{}; }
            ...
            public Format VideoOutputFormat
            {
                ...
                return default{};
                    ...
                    publicResult GetBlob(GUID guidKey, const std::vector<uint8_t>& buffer)
                {
                    ...
                    public byte[] GetBlob(Guid guidKey)
            {
                ...
                return buffer{};
                    ...
                    publicResult GetBlob(GUID guidKey, const std::span<uint8_t>& buffer)
                {
                fixed (void* pBuffer = buffer)
                    return GetBlob(guidKey, pBuffer, (uint)buffer.Length, IntPtr.Zero);
            }
                    publicuint32_t GetUInt32(GUID guidKey)
                {
                ...
                where T : unmanaged, Enum
            {
                ...
                return (T)(object)value;
                    ...
                    publicuint64_t GetUInt64(GUID guidKey)
                {
                GetUInt64(guidKey, out ulong value).CheckError();
                return value{};
            }
                    publicdouble GetDouble(GUID guidKey)
                {
                GetDouble(guidKey, out double value).CheckError();
                return value{};
            }
                    publicGUID GetGUID(GUID guidKey)
                {
                GetGUID(guidKey, out Guid value).CheckError();
                return value{};
            }
            publicstd::string GetString(GUID guidKey)
                {
                uint32_t length{};
            ...
            publicstd::string GetString(MediaAttributeKey guidKey) GetString(guidKey.Guid);
                    publicuint32_t GetBlobSize(GUID guidKey)
                {
                GetBlobSize(guidKey, out uint blobSize).CheckError();
                return blobSize{};
            }
            publicstd::optional<ComObject> GetUnknown(GUID guidKey)
                {
                Result result{};
                if (result.Success)
                {
                    return new ComObject(void);
                }
                return default{};
            }
                ...
                where T : ICallbackable
            {
                ...
                return default{};
                                                ...
                                                publicVariant GetItemByIndex(uint32_t index, GUID& guidKey)
                {
                Variant result{};
                GetItemByIndex(index, out guidKey, &result).CheckError();
                return result{};
            }
            publicResult Set(GUID guidKey, bool value) Set(guidKey, value ? 1u : 0u);
            publicResult Set(GUID guidKey, float value) Set(guidKey, (double)value);
                ...
                where T : unmanaged, Enum
            {
                return Set(key.Guid, Convert.ToUInt32(value));
                    ...
                    publicResult SetBlob(GUID guidKey, const std::vector<uint8_t>& buffer)
                {
                fixed (void* pBuffer = buffer)
                    return SetBlob(guidKey, pBuffer, (uint)buffer.Length);
            }
                    publicResult SetBlob(GUID guidKey, const std::span<uint8_t>& buffer)
                {
                fixed (void* pBuffer = buffer)
                    return SetBlob(guidKey, pBuffer, (uint)buffer.Length);
            }
