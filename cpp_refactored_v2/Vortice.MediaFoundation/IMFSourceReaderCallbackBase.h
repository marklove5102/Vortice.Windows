...
namespace MediaFoundation {
        ...
        publicclass IMFSourceReaderCallbackBase : : CallbackBase, IMFSourceReaderCallback
        {
            publicvirtual void OnEvent(SourceReaderIndex streamIndex, IMFMediaEvent @event)
                {
            }
            publicvirtual void OnFlush(SourceReaderIndex streamIndex)
                {
            }
            publicvirtual void OnReadSample(Result hrStatus, SourceReaderIndex streamIndex, SourceReaderFlag streamFlags, int64_t llTimestamp, IMFSample sample)
                {
            }
