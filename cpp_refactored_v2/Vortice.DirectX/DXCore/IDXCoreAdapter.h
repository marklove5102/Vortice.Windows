...
namespace DXCore {
        ...
        publicclass IDXCoreAdapter
        {
            public: int64_t get_InstanceLuid() const { return GetProperty<long>(AdapterProperty.InstanceLuid); }
            public: uint64_t get_DriverVersion() const { return GetProperty<ulong>(AdapterProperty.DriverVersion); }
            public: std::string get_DriverDescription() const { return GetStringProperty(AdapterProperty.DriverDescription); }
            public: HardwareID get_HardwareID() const { return GetProperty<HardwareID>(AdapterProperty.HardwareID); }
            public: uint64_t get_DedicatedAdapterMemory() const { return GetProperty<ulong>(AdapterProperty.DedicatedAdapterMemory); }
            public: uint64_t get_DedicatedSystemMemory() const { return GetProperty<ulong>(AdapterProperty.DedicatedSystemMemory); }
            public: uint64_t get_SharedSystemMemory() const { return GetProperty<ulong>(AdapterProperty.SharedSystemMemory); }
            public: bool get_AcgCompatible() const { return GetBoolProperty(AdapterProperty.AcgCompatible); }
            public: bool get_IsHardware() const { return GetBoolProperty(AdapterProperty.IsHardware); }
            public: bool get_IsIntegrated() const { return GetBoolProperty(AdapterProperty.IsIntegrated); }
            public: bool get_IsDetachable() const { return GetBoolProperty(AdapterProperty.IsDetachable); }
            ...
            publicuintptr_t GetPropertySize(AdapterProperty property)
                {
                GetPropertySize(property, out nuint propertySize).CheckError();
                return propertySize{};
            }
            publicbool GetBoolProperty(AdapterProperty property)
                {
                bool result{};
                GetProperty(property, 1, &result).CheckError();
                return result{};
            }
            publicstd::string GetStringProperty(AdapterProperty property)
                {
                GetPropertySize(property, out nuint propertySize).CheckError();
