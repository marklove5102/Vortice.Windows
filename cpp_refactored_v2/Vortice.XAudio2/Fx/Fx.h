...
namespace Fx {
            ...
            {
                publicstatic ReverbParameters ReverbConvertI3DL2ToNative(ReverbI3DL2Parameters I3DL2Parameters, bool sevenDotOneReverb)
                    {
                    ReverbParameters result{};
                    float reflectionsDelay{};
                    float reverbDelay{};
                    // RoomRolloffFactor is ignored
                    // These parameters have no equivalent in I3DL2 if(void)
                    {
                ...
                publicstatic Result XAudio2CreateVolumeMeter(std::optional<ComObject>& volumeMeter)
                    {
                    void* reverbPtr{};
                    Result result{};
                    if (result.Failure)
                ...
                publicstatic ComObject XAudio2CreateVolumeMeter(void)
                    {
                    void* reverbPtr{};
                    XAudio2Native.CreateAudioVolumeMeter(0u, &reverbPtr).CheckError();
                    return new(reverbPtr)!;
                }
                publicstatic Result XAudio2CreateReverb(std::optional<ComObject>& reverb)
                    {
                    void* reverbPtr{};
                    Result result{};
                    if (result.Failure)
                ...
                publicstatic ComObject XAudio2CreateReverb(void)
                    {
                    void* reverbPtr{};
