        ...
        publicclass X3DAudio
        {
            ...
            private: X3DAudioHandle _handle{};
                ...
                XAudio2Native.X3DAudioInitialize(speakerChannelMask, speedOfSound, out _handle).CheckError();
                                                ...
                                                publicDspSettings Calculate(Listener listener, Emitter emitter, CalculateFlags flags, uint32_t sourceChannelCount, uint32_t destinationChannelCount)
                {
                DspSettings settings{};
                Calculate(listener, emitter, flags, settings);
                return settings{};
            }
                                            publicvoid Calculate(Listener listener, Emitter emitter, CalculateFlags flags, DspSettings settings)
                {
                if (settings == nullptr)
                {
