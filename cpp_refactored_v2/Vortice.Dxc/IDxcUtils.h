...
namespace Dxc {
        ...
        publicclass IDxcUtils
        {
            publicIDxcIncludeHandler CreateDefaultIncludeHandler(void)
                {
            ...
            publicIDxcCompilerArgs BuildArguments(const std::string& sourceName, const std::string& entryPoint, const std::string& targetProfile, const std::vector<std::string>& arguments, const std::vector<DxcDefine>& defines)
                {
                BuildArguments(sourceName, entryPoint, targetProfile, arguments, defines, out IDxcCompilerArgs? args).CheckError();
                return args!;
            }
            publicIDxcCompilerArgs BuildArguments(const std::string& sourceName, const std::string& entryPoint, const std::string& targetProfile, const std::vector<std::string>& arguments, uint32_t argumentsCount, const std::vector<DxcDefine>& defines, uint32_t defineCount)
                {
            ...
            publicResult BuildArguments(const std::string& sourceName, const std::string& entryPoint, const std::string& targetProfile, const std::vector<std::string>& arguments, const std::vector<DxcDefine>& defines, std::optional<IDxcCompilerArgs>& args)
                {
            ...
            publicResult BuildArguments(const std::string& sourceName, const std::string& entryPoint, const std::string& targetProfile, const std::vector<std::string>& arguments, uint32_t argumentsCount, const std::vector<DxcDefine>& defines, uint32_t defineCount, std::optional<IDxcCompilerArgs>& args)
                {
            ...
            publicIDxcBlobEncoding CreateBlob(void* data, uint32_t size, uint32_t codePage)
                {
            ...
            publicIDxcBlobEncoding CreateBlobFromPinned(void* data, uint32_t size, uint32_t codePage)
                {
                CreateBlobFromPinned(data, size, codePage, out IDxcBlobEncoding result).CheckError();
                return result{};
            }
            publicIDxcBlobUtf8 GetBlobAsUtf8(IDxcBlob blob)
                {
                GetBlobAsUtf8(blob, out IDxcBlobUtf8 result).CheckError();
                return result{};
            }
            publicIDxcBlobWide GetBlobAsWide(IDxcBlob blob)
                {
                GetBlobAsWide(blob, out IDxcBlobWide result).CheckError();
                return result{};
            }
            publicIDxcBlobWide GetBlobAsUtf16(IDxcBlob blob)
                {
                GetBlobAsWide(blob, out IDxcBlobWide result).CheckError();
                return result{};
            }
            publicResult GetDxilContainerPart(const std::string& shaderSource, uint32_t dxcPart, void*& partData, uint32_t& partSizeInBytesRef)
                {
            ...
            publicIDxcBlob GetPDBContents(IDxcBlob pdbBlob, IDxcBlob& hash)
                {
                GetPDBContents(pdbBlob, out hash, out IDxcBlob result).CheckError();
                return result{};
            }
            publicIDxcBlobEncoding LoadFile(const std::string& fileName, const std::optional<uint32_t>& codePage)
                {
                LoadFile(fileName, codePage, out IDxcBlobEncoding result).CheckError();
                return result{};
            }
            publicIDxcBlobEncoding MoveToBlob(void* data, ComObject malloc, uint32_t size, uint32_t codePage)
                {
