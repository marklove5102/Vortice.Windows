...
namespace Dxc {
        ...
        publicclass IDxcContainerReflection
        {
            publicuint32_t FindFirstPartKind(uint32_t kind)
                {
                FindFirstPartKind(kind, out uint result).CheckError();
                return result{};
            }
            publicuint32_t GetPartKind(uint32_t index)
                {
                GetPartKind(index, out uint result).CheckError();
                return result{};
            }
            publicIDxcBlob GetPartContent(uint32_t index)
                {
                GetPartContent(index, out IDxcBlob result).CheckError();
                return result{};
            }
            publicuint32_t GetPartCount(void)
                {
