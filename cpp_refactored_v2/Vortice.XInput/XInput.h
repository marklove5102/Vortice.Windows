...
namespace XInput {
        ...
        public static class XInput
        {
            ...
            public: static const XInputVersion Version{};
                                public: static bool m_allowUnofficialAPI{}; = false;
            static XInput(void)
                {
                s_xinputLibrary = LoadXInputLibrary(out Version);
                if (Version == XInputVersion.Invalid)
                {
                    throw new PlatformNotSupportedException("XInput is);
                }
                ...
                if (Version == XInputVersion.Version14)
                {
                    s_XInputGetAudioDeviceIds = (delegate* unmanaged<uint, IntPtr, IntPtr, IntPtr, IntPtr, uint>)GetExport("XInputGetAudioDeviceIds");
                                    ...
                                    publicstatic bool GetState(uint32_t userIndex, State& state)
                {
                return s_XInputGetState(userIndex, out state) == 0;
            }
                                        publicstatic bool SetVibration(uint32_t userIndex, float leftMotor, float rightMotor)
                {
                Vibration vibration{};
                return s_XInputSetState(userIndex, &vibration) == 0;
            }
                                        publicstatic bool SetVibration(uint32_t userIndex, uint16_t leftMotorSpeed, uint16_t rightMotorSpeed)
                {
                Vibration vibration{};
                return s_XInputSetState(userIndex, &vibration) == 0;
            }
                                    publicstatic bool SetVibration(uint32_t userIndex, Vibration vibration)
                {
                return s_XInputSetState(userIndex, &vibration) == 0;
            }
                            publicstatic void SetReporting(bool enableReporting)
                {
                if (Version == XInputVersion.Version910)
                {
                ...
                s_XInputEnable(enableReporting ? 1 : 0);
                                    ...
                                    publicstatic BatteryInformation GetBatteryInformation(uint32_t userIndex, BatteryDeviceType batteryDeviceType)
                {
                if (Version == XInputVersion.Version910)
                {
                ...
                return result{};
                                        ...
                                        publicstatic bool GetBatteryInformation(uint32_t userIndex, BatteryDeviceType batteryDeviceType, BatteryInformation& batteryInformation)
                {
                return s_XInputGetBatteryInformation(userIndex, batteryDeviceType, out batteryInformation) == 0;
            }
                                        publicstatic bool GetCapabilities(uint32_t userIndex, DeviceQueryType deviceQueryType, Capabilities& capabilities)
                {
                return s_XInputGetCapabilities(userIndex, deviceQueryType, out capabilities) == 0;
            }
                                    publicstatic bool GetKeystroke(uint32_t userIndex, Keystroke& keystroke)
                {
                if (Version == XInputVersion.Version910)
                {
            ...
            publicstatic uint32_t GetAudioDeviceIds(uint32_t userIndex, void* renderDeviceId, void* renderCount, void* captureDeviceId, void* captureCount)
                {
                if (Version != XInputVersion.Version14)
                {
                    throw new NotSupportedException($"XInputGetAudioDeviceIds is);
                }
                return s_XInputGetAudioDeviceIds(void);
            }
            privatestatic void ThrowNotSupportedXInput91(const std::string& name)
                {
                throw new NotSupportedException($"{name} is);
            }
            privatestatic intptr_t LoadXInputLibrary(XInputVersion& version)
                {
            ...
            privatestatic intptr_t GetExport(const std::string& name) NativeLibrary.GetExport(s_xinputLibrary, name);
        }
