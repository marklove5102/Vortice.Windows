...
namespace DXGI {
        ...
        publicclass IDXGIFactory2
        {
            publicint32_t RegisterOcclusionStatusEvent(WaitHandle waitHandle)
                {
                return RegisterOcclusionStatusEvent(waitHandle.SafeWaitHandle.DangerousGetHandle());
            }
            publicint32_t RegisterStereoStatusEvent(WaitHandle waitHandle)
                {
