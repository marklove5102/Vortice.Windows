...
namespace WinUI {
        ...
        publicclass ISurfaceImageSourceNative : : ComObject
        {
            public ISurfaceImageSourceNative(nint nativePtr)
                : base(nativePtr)
            {
            }
            public static explicit operator ISurfaceImageSourceNative?(nint nativePtr) => (nativePtr == 0) ? default : new ISurfaceImageSourceNative(void);
        #if WINDOWS
            ...
            publicResult SetDevice(IDXGIDevice device)
                {
                void* devicePtr{};
                Result result{};
                GC.KeepAlive(device);
            ...
            publicResult BeginDraw(const RectI& updateRect, std::optional<IDXGISurface>& surface, Int2& offset)
                {
        ...
        #if WINDOWS
            publicResult BeginDraw(const Windows.Foundation.Rect& updateRect, std::optional<IDXGISurface>& surface, Windows.Foundation.Point& offset)
                {
                RawRect updateRectRaw{};
                intptr_t surfacePtr{};
                Int2 offsetCall{};
