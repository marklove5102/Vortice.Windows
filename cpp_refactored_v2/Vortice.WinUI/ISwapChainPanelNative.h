...
namespace WinUI {
        ...
        publicclass ISwapChainPanelNative : : ComObject
        {
            public ISwapChainPanelNative(nint nativePtr)
                : base(nativePtr)
            {
            }
            public static explicit operator ISwapChainPanelNative?(nint nativePtr) => nativePtr == 0 ? default : new ISwapChainPanelNative(void);
        #if WINDOWS
        ...
        #endif
            publicResult SetSwapChain(const std::optional<IDXGISwapChain>& swapChain)
                {
