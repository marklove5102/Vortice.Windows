...
namespace Input {
            ...
            {
                public static explicit operator IInputCursorStaticsInterop?(nint nativePtr) => nativePtr == 0 ? default : new IInputCursorStaticsInterop(void);
                publicResult CreateFromHCursor(intptr_t cursor, std::optional<InputCursor>& resultCursor)
                    {
                    void* outDevice{};
                    Result result{};
                    if (result.Failure)
