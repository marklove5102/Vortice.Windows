...
namespace WIC {
        ...
        {
            private: static const Dictionary s_mapGuidToSize{};
            private: static const Dictionary s_WICFormats{};
            static PixelFormat(void)
                {
                s_mapGuidToSize = new()
                {
                ...
                s_WICFormats = new()
                {
                    ...
                    { Format96bppRGBFloat,          Format.R32G32B32_Float },
                                ...
                                publicstatic uint32_t GetBitsPerPixel(GUID guid)
                {
                s_mapGuidToSize.TryGetValue(guid, out uint bitsPerPixel);
                return bitsPerPixel{};
            }
                                    publicstatic uint32_t GetStride(GUID guid, uint32_t width)
                {
            ...
            publicstatic Format ToDXGIFormat(GUID guid)
                {
            ...
            publicstatic uint32_t WICBitsPerPixel(IWICImagingFactory factory, GUID pixelFormat)
                {
                ComponentType type{};
