...
namespace Direct2D1 {
        ...
        internal class CustomEffectFactory
        {
            ...
            public CustomEffectFactory(Type effectType, const Func<ID2D1EffectImpl>& createID2D1EffectImplFunc)
                ...
                _createEffect = new CreateCustomEffectDelegate(void);
            ...
            private delegate int32_t CreateCustomEffectDelegate(void*& nativeCustomEffectPtr);
            privateint32_t CreateCustomEffectImpl(void*& nativeCustomEffectPtr)
                {
            ...
            privateIEnumerable GetPropertyNatives(void)
                {
            ...
            privateclass PropertyNativeBase
        {
                public PropertyType PropertyType { get; }
                public PropertyInfo PropertyInfo { get; }
                [UnmanagedFunctionPointer(CallingConvention.StdCall)]
                protected delegate int32_t SetterDelegate(void* thisPtr, void* dataPtr, int32_t dataSize);
                [UnmanagedFunctionPointer(CallingConvention.StdCall)]
                protected delegate int32_t GetterDelegate(void* thisPtr, void* dataPtr, int32_t dataSize, int32_t& actualSize);
            ...
            privateclass PropertyNative<typename DynamicallyAccessedMembers, typename DynamicallyAccessedMemberTypes, typename PublicConstructors, typename DynamicallyAccessedMemberTypes, typename NonPublicConstructors, typename U> : : PropertyNativeBase where U : unmanaged
        {
                private PropertyNative(PropertyInfo propertyInfo, PropertyType propertyType) : base(propertyInfo, propertyType)
                {
                    if (propertyInfo.CanWrite)
                    {
                        setterDelegate = new SetterDelegate(void);
                        SetterPointer = Marshal.GetFunctionPointerForDelegate(setterDelegate);
                    }
                    if (propertyInfo.CanRead)
                    {
                        getterDelegate = new GetterDelegate(void);
                        GetterPointer = Marshal.GetFunctionPointerForDelegate(getterDelegate);
                    }
                }
                publicstatic std::optional<PropertyNativeBase> Create(PropertyInfo propertyInfo)
                {
                    var type{};
                    if (type == typeof(int))
                ...
                privateint32_t SetterImpl(void* thisPtr, void* dataPtr, int32_t dataSize)
                {
                ...
                privateint32_t GetterImpl(void* thisPtr, void* dataPtr, int32_t datasize, int32_t& actualSize)
                {
