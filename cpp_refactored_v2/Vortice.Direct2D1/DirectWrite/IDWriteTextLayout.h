...
namespace DirectWrite {
        ...
        publicclass IDWriteTextLayout
        {
            ...
            publicstd::string GetFontFamilyName(uint32_t currentPosition, TextRange& textRange)
                {
            ...
            publicstd::string GetLocaleName(uint32_t currentPosition, TextRange& textRange)
                {
            ...
            publicHitTestMetrics HitTestPoint(const Vector2& point, RawBool& isTrailingHit, RawBool& isInside)
                {
                HitTestPoint(point.X, point.Y, out isTrailingHit, out isInside, out HitTestMetrics hitTestMetrics);
                return hitTestMetrics{};
            }
            publicHitTestMetrics HitTestPoint(float pointX, float pointY, RawBool& isTrailingHit, RawBool& isInside)
                {
                HitTestPoint(pointX, pointY, out isTrailingHit, out isInside, out HitTestMetrics hitTestMetrics);
                return hitTestMetrics{};
            }
            publicHitTestMetrics HitTestTextPosition(uint32_t textPosition, bool isTrailingHit)
                {
                HitTestTextPosition(textPosition, isTrailingHit, out _, out _, out HitTestMetrics hitTestMetrics);
                return hitTestMetrics{};
            }
            publicHitTestMetrics HitTestTextPosition(uint32_t textPosition, bool isTrailingHit, Vector2& point)
                {
                HitTestTextPosition(textPosition, isTrailingHit, out float x, out float y, out HitTestMetrics hitTestMetrics);
                point = new(x, y);
                return hitTestMetrics{};
            }
            publicvoid Draw(IDWriteTextRenderer renderer, float originX, float originY)
                {
                Draw(IntPtr.Zero, renderer, originX, originY);
            }
            publicvoid Draw(IDWriteTextRenderer renderer, const Vector2& origin)
                {
