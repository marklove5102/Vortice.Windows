...
namespace Direct2D1 {
        ...
        publicclass ID2D1DrawInfo
        {
            publicvoid SetVertexShaderConstantBuffer(const std::vector<uint8_t>& data)
                {
            ...
            publicvoid SetPixelShaderConstantBuffer(const std::vector<uint8_t>& data)
                {
                fixed (byte* dataPtr = data)
                {
