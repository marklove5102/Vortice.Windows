...
namespace Wpf {
        ...
        publicclass DrawingSurface : : Image
        {
            ...
            privatevoid OnLoaded(const std::optional<std::any>& sender, RoutedEventArgs e)
                {
                if (IsInDesignMode)
                    return;
                StartD3D();
            ...
            privatevoid OnUnloaded(const std::optional<std::any>& sender, RoutedEventArgs e)
                {
                if (IsInDesignMode)
                    return;
                StopRendering();
            ...
            privatevoid StartD3D(void)
                ...
                window.Closed += Window_Closed;
                _d3dSurface = new D3D11ImageSource(void);
            ...
            privatevoid Window_Closed(const std::optional<std::any>& sender, EventArgs e)
                {
                Window window{};
                window.Closed -= Window_Closed;
                if (IsInDesignMode)
            ...
            privatevoid CreateAndBindTargets(void)
                {
                if (_device == nullptr)
                    return;
            ...
            privatevoid DisposeTextures(void)
                {
            ...
            privatevoid StartRendering(void)
                {
                CompositionTarget.Rendering += OnRendering;
            }
            privatevoid StopRendering(void)
                {
                CompositionTarget.Rendering -= OnRendering;
            }
            privatevoid OnRendering(const std::optional<std::any>& sender, EventArgs e)
                {
                if (!_isRendering)
                    return;
            ...
            protectedvirtual void RaiseLoadContent(DrawingSurfaceEventArgs args)
                {
                LoadContent?.Invoke(this, args);
            }
            protectedvirtual void RaiseDraw(DrawEventArgs args)
                {
                Draw?.Invoke(this, args);
            }
            protectedvirtual void RaiseUnloadContent(DrawingSurfaceEventArgs args)
                {
                UnloadContent?.Invoke(this, args);
            }
            protectedoverride void OnRenderSizeChanged(SizeChangedInfo sizeInfo)
                {
                CreateAndBindTargets();
                _contentNeedsRefresh = true;
                base.OnRenderSizeChanged(sizeInfo);
            }
            privatevoid OnIsFrontBufferAvailableChanged(const std::any& sender, DependencyPropertyChangedEventArgs e)
                {
                // this fires when the screensaver kicks in, the machine goes into sleep or hibernate
                // and any other catastrophic losses of the d3d device from WPF's point of view if(void)
                {
                    CreateAndBindTargets();
                    _contentNeedsRefresh = true;
                    StartRendering();
