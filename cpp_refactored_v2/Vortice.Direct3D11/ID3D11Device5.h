...
namespace Direct3D11 {
        ...
        publicclass ID3D11Device5
        {
            publicID3D11Fence CreateFence(uint64_t initialValue, FenceFlags flags)
                {
                CreateFence(initialValue, flags, typeof(ID3D11Fence).GUID, out IntPtr nativePtr).CheckError();
                return new ID3D11Fence(void);
            }
            ...
            publicID3D11Fence OpenSharedFence(void* fenceHandle)
                {
