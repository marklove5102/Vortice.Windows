...
namespace Direct3D11 {
        ...
        publicstruct Texture2DDescription
        {
                                                                    public Texture2DDescription(Format format, uint32_t width, uint32_t height, uint32_t arraySize, uint32_t mipLevels, BindFlags bindFlags, ResourceUsage usage, CpuAccessFlags cpuAccessFlags, uint32_t sampleCount, uint32_t sampleQuality, ResourceOptionFlags miscFlags)
                {
                ...
                MiscFlags = miscFlags;
                                ...
                                publicuint32_t GetWidth(uint32_t mipLevel)
                {
                return (mipLevel == 0) || (mipLevel < MipLevels) ? Math.Max(1, Width >> (int)mipLevel) : 0;
            }
                                publicuint32_t GetHeight(uint32_t mipLevel)
                {
