...
namespace Direct3D11on12 {
        ...
        publicclass ID3D11On12Device
        {
            ...
            publicvoid AcquireWrappedResources(const std::vector<ID3D11Resource>& resources)
                {
                AcquireWrappedResources(resources, (uint)resources.Length);
            }
            publicvoid ReleaseWrappedResources(const std::vector<ID3D11Resource>& resources)
                {
